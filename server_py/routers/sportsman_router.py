...
from repositories import sportsman

from db.database import get_db

router = APIRouter(prefix="/api/sportsmen", tags=["sportsman"])


# Create
@router.post("/", response_model=SportsmanModelDisplay)
async def create_element(request: SportsmanModelDisplay, db: Session = Depends(get_db)):
    element = sportsman.create(db=db, request=request)
    if element is None:
        raise HTTPException(status_code=400, detail="ERROR")
    return element


@router.get("/", response_model=list[SportsmanModelDisplay])
async def get_all_elements(db: Session = Depends(get_db)):
    elements_list = sportsman.get_all_elements(db=db)
